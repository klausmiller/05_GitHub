...
hist(datapoints_segment, breaks = 1000, xlim = c(0,30000))
...
hist(datapoints_cookie$segmentassignments, breaks = 1000, xlim = c(0,1000))
...
# There is hardly any overlap of timestamps across users. I also cross-checked with Excel. The share of overlappen timestamps is approximately 0.000536 (1 - 259048 / 259187)
...
#test = ifelse(datapoints_cookie_segment[c(grep("Group.1",colnames(datapoints_cookie_segment)):length(colnames(datapoints_cookie_segment)))] == "NA",0,1)
################
...
summarise(datapoints_cookie_segment[c(4:10)])
...
# Explanation: We can start similar as we did in the calculation of segment assignments per period. We only need to add an additional feature for the grouping variable. This will create a big panel file.
...
rm("datapoints_cookie_period_identifier")
...
timelist = as.list(strsplit(gsub(':','_',gsub(' ','_',gsub('-','_',as.character(whole_cookie_frame$period)))),'_'))

# Here, the loop to create the user-time-identifier starts with the user_id_64. We have to get rid of those cookies, which have NA as start or ending date, because we cannot create the period identifier for them.
panel_cookie_frame = subset(whole_cookie_frame, is.na(whole_cookie_frame$period) = FALSE)
cookie_period_id = as.character(whole_cookie_frame$user_id_64)
for (ticker in c(1:cookie_period_granularity)){
  cookie_period_id = paste(cookie_period_id,sapply(timelist, "[[", ticker), sep='_')
  print(ticker)
}
whole_cookie_frame$cookie_period_id = cookie_period_id
...
whole_cookie_frame = merge(whole_cookie_frame, datapoints_cookie_period_segment, by = c("cookie_period_id"), all.x = TRUE)

write.csv(whole_cookie_frame,"/home/kraft/Dropbox/03_Shared/01_KlausMiller_LennartKraft/02_Data/04_output/02_StaFe/panel_data.csv")
...
write(x = r_file, file = paste(substr(working_folder,1,nchar(working_folder)),'03_Shared/01_KlausMiller_LennartKraft/05_GitHub/02_RMarkDown/InformationOnCookies.R', sep = ''))
